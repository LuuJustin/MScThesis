...
class HipXrayClassifier(L.LightningModule):
    ...
    def forward(self, x):
        allocated = torch.cuda.memory_allocated(device=x.device) / 1024 ** 2  # in MB
        reserved = torch.cuda.memory_reserved(device=x.device) / 1024 ** 2  # in MB
        print(
            f"[Forward] Input shape: {x.shape} | Allocated memory: {allocated:.2f} MB | Reserved memory: {reserved:.2f} MB")
        return self.model(x)

    def training_step(self, batch, batch_idx):
        x, y = batch
        # print(f"Train Data on: {x.device}, {y.device}")  # Should print "cuda:0"
    ...
    def validation_step(self, batch, batch_idx):
        x, y = batch
        # print(f"Val Data on: {x.device}, {y.device}")  # Should print "cuda:0"
